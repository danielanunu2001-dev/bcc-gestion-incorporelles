// src/pages/Rapports/RapportAmortissements.jsx
...
const RapportAmortissements = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      setError('');
      const res = await api.get(`/reports/tableau-amortissements?exercice=${exercice}`);
      setData(res.data.amortissements || []);
    } catch (error) {
      console.error('Erreur chargement rapport amortissements:', error);
  ...
  const totalAnnuite = data.reduce((s, i) => s + (i.annuite || 0), 0);
  const totalVNC = data.reduce((s, i) => s + (i.vnc || 0), 0);
  const totalValeurBrute = data.reduce((s, i) => s + (i.valeur_brute || 0), 0);
  const totalCumul = data.reduce((s, i) => s + (i.cumul || 0), 0);
