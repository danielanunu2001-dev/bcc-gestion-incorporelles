  ...
  const renderTree = (parentId = null, level = 0) => {
    ...
    return children.map(compte => (
      <div key={compte.id} className="ms-3">
        <div className="d-flex align-items-center justify-content-between p-2 rounded-2 hover-bg-light" style={{ marginLeft: level * 24, backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: '8px', marginBottom: '4px' }}>
          <div className="d-flex align-items-center flex-grow-1 gap-2 flex-wrap">
            {getComptesByParent(compte.id).length > 0 && <Button variant="link" size="sm" onClick={() => toggleExpand(compte.id)} className="p-0" style={{ color: '#ffffff', textDecoration: 'none' }}>{expandedNodes[compte.id] ? '▼' : '▶'}</Button>}
            {getComptesByParent(compte.id).length === 0 && <div style={{ width: '24px' }} />}
            <code className="px-2 py-1 rounded small" style={{ color: '#ffffff', backgroundColor: 'rgba(255,255,255,0.2)', fontFamily: 'monospace' }}>{compte.code}</code>
            <span className="small" style={{ color: '#ffffff', fontWeight: 'normal' }}>{compte.libelle}</span>
            {compte.typeImmobilisation && <Badge bg={compte.typeImmobilisation === 'incorporelle' ? 'warning' : 'success'} className="bg-opacity-10" style={{ color: '#ffffff', backgroundColor: compte.typeImmobilisation === 'incorporelle' ? 'rgba(255,193,7,0.2)' : 'rgba(25,135,84,0.2)', padding: '4px 8px', border: 'none' }}>{compte.typeImmobilisation === 'incorporelle' ? 'Incorporelle' : 'Corporelle'}</Badge>}
            {compte.typeReserve && <Badge bg="warning" className="bg-opacity-10" style={{ color: '#ffffff', backgroundColor: 'rgba(255,193,7,0.2)', padding: '4px 8px', border: 'none' }}>{compte.typeReserve === 'change' ? 'Réserves de change' : 'Réserves monétaires'}</Badge>}
            {compte.obligatoire && <Badge bg="danger" className="bg-opacity-10" style={{ color: '#ffffff', backgroundColor: 'rgba(220,53,69,0.2)', padding: '4px 8px', border: 'none' }}>Obligatoire</Badge>}
            <Badge bg={compte.actif ? 'success' : 'secondary'} className="bg-opacity-10" style={{ color: '#ffffff', backgroundColor: compte.actif ? 'rgba(25,135,84,0.2)' : 'rgba(108,117,125,0.2)', padding: '4px 8px', border: 'none' }}>{compte.actif ? 'Actif' : 'Inactif'}</Badge>
          </div>
          <div className="d-flex gap-1">
            {onSelectCompte && <Button variant="outline-info" size="sm" onClick={() => onSelectCompte(compte)} title="Lier" style={{ borderColor: '#0dcaf0', color: '#0dcaf0', backgroundColor: 'transparent' }}><FiLink size={14} /></Button>}
            {can('edit') && <Button variant="outline-warning" size="sm" onClick={() => handleEditCompte(compte)} title="Modifier" style={{ borderColor: '#ffc107', color: '#ffc107', backgroundColor: 'transparent' }}><FiEdit2 size={14} /></Button>}
            {can('edit') && <Button variant={compte.actif ? 'outline-secondary' : 'outline-success'} size="sm" onClick={() => handleToggleActif(compte.id)} title={compte.actif ? 'Désactiver' : 'Activer'} style={compte.actif ? { borderColor: '#6c757d', color: '#6c757d', backgroundColor: 'transparent' } : { borderColor: '#198754', color: '#198754', backgroundColor: 'transparent' }}>{compte.actif ? <FiXCircle size={14} /> : <FiCheckCircle size={14} />}</Button>}
            {can('delete') && !compte.obligatoire && <Button variant="outline-danger" size="sm" onClick={() => handleDeleteCompte(compte.id)} title="Supprimer" style={{ borderColor: '#dc3545', color: '#dc3545', backgroundColor: 'transparent' }}><FiTrash2 size={14} /></Button>}
          </div>
        </div>
      ...
      <>
        ...
        <div style={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', minHeight: '100vh' }}>
          <Container className="py-5 text-center fade-slide-up">
            ...
            <p style={{ color: 'white' }}>Chargement du plan comptable BCC...</p>
          </Container>
        </div>
      </>
    );
      ...
      <div style={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', minHeight: '100vh' }}>
        <Container fluid className="py-4 px-3 px-md-4 fade-slide-up" style={{ maxWidth: '1400px' }}>
          ...
          <div className="d-flex justify-content-between align-items-start flex-wrap gap-3 mb-4">
            <div>
              ...
              <div className="d-flex align-items-center gap-3">
                ...
                <div>
                  <h1 className="h3 fw-bold mb-1" style={{ color: 'white' }}>Plan Comptable - Banque Centrale du Congo</h1>
                  <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: '0.875rem' }}>Référentiel comptable selon les normes de la BCC</p>
                </div>
              </div>
            </div>
            <div className="d-flex gap-2 flex-wrap">
              <Button variant="light" onClick={() => setDarkMode(!darkMode)} className="d-flex align-items-center gap-2" style={{ backgroundColor: 'rgba(255,255,255,0.2)', border: '1px solid rgba(255,255,255,0.3)', color: 'white' }}>{darkMode ? <FiSun size={16} /> : <FiMoon size={16} />}</Button>
              <Button variant="info" onClick={() => setShowDashboard(!showDashboard)} className="d-flex align-items-center gap-2" style={{ backgroundColor: '#0dcaf0', borderColor: '#0dcaf0', color: 'white' }}><FiBarChart2 size={16} /> Dashboard</Button>
              <Button variant="success" onClick={handleExportJSON} className="d-flex align-items-center gap-2" style={{ backgroundColor: '#198754', borderColor: '#198754', color: 'white' }}><FiDownload size={16} /> JSON</Button>
              <Button variant="success" onClick={handleExportCSV} className="d-flex align-items-center gap-2" style={{ backgroundColor: '#198754', borderColor: '#198754', color: 'white' }}>CSV</Button>
              {can('reset') && <Button variant="danger" onClick={handleReset} className="d-flex align-items-center gap-2" style={{ backgroundColor: '#dc3545', borderColor: '#dc3545', color: 'white' }}><FiRefreshCw size={16} /> Réinitialiser</Button>}
            </div>
          </div>

          {/* Messages */}
          {success && <Alert variant="success" dismissible onClose={() => setSuccess('')} className="mb-3" style={{ backgroundColor: '#d1fae5', borderColor: '#10b981', color: '#065f46' }}><FiCheckCircle className="me-2" />{success}</Alert>}
          {error && <Alert variant="danger" dismissible onClose={() => setError('')} className="mb-3" style={{ backgroundColor: '#fee2e2', borderColor: '#ef4444', color: '#991b1b' }}><FiXCircle className="me-2" />{error}</Alert>}
            ...
            <Card className="border-0 shadow-sm rounded-3 mb-4" style={{ backgroundColor: 'rgba(255,255,255,0.95)' }}>
              <Card.Body>
                <div className="d-flex justify-content-between align-items-center mb-3">
                  <h3 className="h6 fw-semibold mb-0" style={{ color: '#000000' }}>Tableau de bord - Plan comptable BCC</h3>
                  <Button variant="link" onClick={() => setShowDashboard(false)} className="text-secondary p-0"><FiXCircle size={20} style={{ color: '#6c757d' }} /></Button>
                </div>
                <Row className="g-3">
                  <Col xs={6} md={3}><div className="bg-light rounded-3 p-3 text-center" style={{ backgroundColor: '#f8f9fa' }}><div className="h2 mb-0" style={{ color: '#198754' }}>{stats.total}</div><small className="text-muted">Total comptes</small></div></Col>
                  <Col xs={6} md={3}><div className="bg-light rounded-3 p-3 text-center" style={{ backgroundColor: '#f8f9fa' }}><div className="h2 mb-0" style={{ color: '#198754' }}>{stats.actifs}</div><small className="text-muted">Comptes actifs</small></div></Col>
                  <Col xs={6} md={3}><div className="bg-light rounded-3 p-3 text-center" style={{ backgroundColor: '#f8f9fa' }}><div className="h2 mb-0" style={{ color: '#0d6efd' }}>{stats.classe2}</div><small className="text-muted">Immobilisations</small></div></Col>
                  <Col xs={6} md={3}><div className="bg-light rounded-3 p-3 text-center" style={{ backgroundColor: '#f8f9fa' }}><div className="h2 mb-0" style={{ color: '#ffc107' }}>{stats.immobilisationsCorporelles}</div><small className="text-muted">Corporelles</small></div></Col>
                </Row>
              </Card.Body>
            </Card>
          )}
          ...
          <Card className="border-0 shadow-sm rounded-3 mb-4" style={{ backgroundColor: 'rgba(255,255,255,0.95)' }}>
            <Card.Body className="p-0">
              <Nav variant="tabs" defaultActiveKey="comptes" className="px-3 pt-2">
                <Nav.Item><Nav.Link eventKey="comptes" onClick={() => setActiveTab('comptes')} className="d-flex align-items-center gap-2" style={{ color: '#000000' }}><FiBook size={14} /> Comptes</Nav.Link></Nav.Item>
                <Nav.Item><Nav.Link eventKey="ajout" onClick={() => setActiveTab('ajout')} className="d-flex align-items-center gap-2" style={{ color: '#000000' }}><FiPlus size={14} /> Ajouter un compte</Nav.Link></Nav.Item>
              </Nav>
            </Card.Body>
          </Card>
            ...
            <Card className="border-0 shadow-sm rounded-3 mb-4" style={{ backgroundColor: 'rgba(255,255,255,0.95)' }}>
              <Card.Body className="p-4">
                <div className="d-flex align-items-center gap-2 mb-3">
                  <div className="rounded-circle bg-success bg-opacity-10 p-2 d-flex align-items-center justify-content-center" style={{ width: '36px', height: '36px', backgroundColor: '#d1fae5' }}><FiPlus size={20} style={{ color: '#198754' }} /></div>
                  <div><h3 className="h6 fw-semibold mb-0" style={{ color: '#000000' }}>Ajouter un compte BCC</h3><p className="small text-muted mb-0">Créez un nouveau compte comptable</p></div>
                </div>
                <Row className="g-3 align-items-end">
                  <Col md={3}><Form.Group><Form.Label className="fw-semibold small" style={{ color: '#000000' }}>Classe</Form.Label><Form.Select value={newCompte.classe} onChange={(e) => setNewCompte({ ...newCompte, classe: e.target.value })} style={{ color: '#000000', backgroundColor: 'white' }}><option value="1">Classe 1 - Capitaux propres</option><option value="2">Classe 2 - Immobilisations</option><option value="3">Classe 3 - Stocks monétaires</option><option value="4">Classe 4 - Opérations État</option><option value="5">Classe 5 - Trésorerie</option><option value="6">Classe 6 - Charges</option><option value="7">Classe 7 - Produits</option></Form.Select></Form.Group></Col>
                  <Col md={2}><Form.Group><Form.Label className="fw-semibold small" style={{ color: '#000000' }}>Code</Form.Label><Form.Control type="text" placeholder="Ex: 421" value={newCompte.code} onChange={(e) => setNewCompte({ ...newCompte, code: e.target.value })} style={{ color: '#000000', backgroundColor: 'white' }} /></Form.Group></Col>
                  <Col md={4}><Form.Group><Form.Label className="fw-semibold small" style={{ color: '#000000' }}>Libellé</Form.Label><Form.Control type="text" placeholder="Ex: Opérations de refinancement" value={newCompte.libelle} onChange={(e) => setNewCompte({ ...newCompte, libelle: e.target.value })} style={{ color: '#000000', backgroundColor: 'white' }} /></Form.Group></Col>
                  <Col md={3}><Button variant="success" onClick={handleAddCompte} className="w-100 d-flex align-items-center justify-content-center gap-2" style={{ backgroundColor: '#198754', borderColor: '#198754', color: 'white' }}><FiPlus size={14} /> Ajouter</Button></Col>
                </Row>
              </Card.Body>
            </Card>
          )}
            ...
            <Card.Body>
              <div className="d-flex gap-3 flex-wrap align-items-center">
                <div className="flex-grow-1" style={{ minWidth: '250px' }}>
                  <InputGroup><InputGroup.Text className="bg-white border-end-0"><FiSearch style={{ color: '#6c757d' }} /></InputGroup.Text><Form.Control type="text" placeholder="Rechercher par code ou libellé..." onChange={(e) => debouncedSearch(e.target.value)} className="border-start-0" style={{ color: '#000000', backgroundColor: 'white' }} /></InputGroup>
                </div>
                <div className="d-flex align-items-center gap-2 px-3 py-2 rounded-3 border" style={{ backgroundColor: '#f8fafc', borderColor: '#dee2e6' }}><FiFilter size={14} style={{ color: '#6c757d' }} /><Form.Select value={filterClasse} onChange={(e) => setFilterClasse(e.target.value)} className="border-0 bg-transparent w-auto" style={{ fontSize: '0.875rem', color: '#000000' }}><option value="all">Toutes classes</option><option value="1">Classe 1</option><option value="2">Classe 2</option><option value="3">Classe 3</option><option value="4">Classe 4</option><option value="5">Classe 5</option><option value="6">Classe 6</option><option value="7">Classe 7</option></Form.Select></div>
                <div className="d-flex align-items-center gap-2 px-3 py-2 rounded-3 border" style={{ backgroundColor: '#f8fafc', borderColor: '#dee2e6' }}><Form.Select value={filterActif} onChange={(e) => setFilterActif(e.target.value)} className="border-0 bg-transparent w-auto" style={{ fontSize: '0.875rem', color: '#000000' }}><option value="all">Tous statuts</option><option value="actif">Actifs</option><option value="inactif">Inactifs</option></Form.Select></div>
                <div className="btn-group" role="group">
                  <Button variant={viewMode === 'tree' ? 'primary' : 'outline-secondary'} size="sm" onClick={() => setViewMode('tree')} style={viewMode === 'tree' ? { backgroundColor: '#0d6efd', color: 'white' } : { borderColor: '#6c757d', color: '#6c757d' }}><FiFolder size={14} /> Arborescence</Button>
                  <Button variant={viewMode === 'list' ? 'primary' : 'outline-secondary'} size="sm" onClick={() => setViewMode('list')} style={viewMode === 'list' ? { backgroundColor: '#0d6efd', color: 'white' } : { borderColor: '#6c757d', color: '#6c757d' }}><FiList size={14} /> Liste</Button>
                </div>
              </div>
            </Card.Body>
          </Card>

          {/* Liste des comptes - Vue Arborescence */}
          {filteredComptes.length === 0 ? (
            ...
            <Card className="border-0 shadow-sm rounded-3 overflow-hidden" style={{ backgroundColor: 'transparent' }}>
            ...
            <Card className="border-0 shadow-sm rounded-3 overflow-hidden" style={{ backgroundColor: 'rgba(255,255,255,0.95)' }}>
              <Card.Body className="p-0">
                <div className="p-3 border-bottom d-flex justify-content-between align-items-center flex-wrap gap-2" style={{ borderBottomColor: '#dee2e6' }}>
                  <h3 className="h6 fw-semibold mb-0" style={{ color: '#000000' }}>Liste des comptes</h3>
                  <Badge bg="secondary" className="px-2 py-1" style={{ backgroundColor: '#6c757d', color: 'white' }}>{filteredComptes.length} compte(s)</Badge>
                </div>
                  ...
                  <Table hover className="align-middle mb-0">
                    <thead className="table-light" style={{ backgroundColor: '#f8f9fa' }}>
                      <tr>
                        ...
                        <th style={{ color: '#000000' }}>Libellé</th>
                        <th style={{ color: '#000000' }}>Classe</th>
                        <th style={{ color: '#000000' }}>Statut</th>
                    ...
                    <tbody>
                      {filteredComptes.map(compte => (
                        <tr key={compte.id}>
                          <td>{editingId === compte.id ? <Form.Control type="text" size="sm" value={editingCompte.code} onChange={(e) => setEditingCompte({ ...editingCompte, code: e.target.value })} style={{ width: '100px', color: '#000000', backgroundColor: 'white' }} /> : <code className="bg-light px-2 py-1 rounded" style={{ color: '#000000', backgroundColor: '#f8f9fa' }}>{compte.code}</code>}</td>
                          <td>{editingId === compte.id ? <Form.Control type="text" size="sm" value={editingCompte.libelle} onChange={(e) => setEditingCompte({ ...editingCompte, libelle: e.target.value })} className="w-100" style={{ color: '#000000', backgroundColor: 'white' }} /> : <span style={{ color: '#000000' }}>{compte.libelle}</span>}</td>
                          <td><Badge bg="secondary" className="bg-opacity-10" style={{ color: '#000000', backgroundColor: '#e9ecef', padding: '4px 8px' }}>Classe {compte.classe}</Badge></td>
                          <td><Badge bg={compte.actif ? 'success' : 'secondary'} className="bg-opacity-10" style={{ color: '#000000', backgroundColor: compte.actif ? '#d1fae5' : '#e5e7eb', padding: '4px 8px' }}>{compte.actif ? 'Actif' : 'Inactif'}</Badge></td>
                          <td><div className="btn-group btn-group-sm">
                            {editingId === compte.id ? <><Button variant="success" onClick={() => handleSaveEdit(compte.id)} style={{ backgroundColor: '#198754', borderColor: '#198754', color: 'white' }}><FiCheckCircle size={14} /></Button><Button variant="secondary" onClick={handleCancelEdit} style={{ backgroundColor: '#6c757d', borderColor: '#6c757d', color: 'white' }}><FiXCircle size={14} /></Button></> : <>{onSelectCompte && <Button variant="outline-info" onClick={() => onSelectCompte(compte)} style={{ borderColor: '#0dcaf0', color: '#0dcaf0' }}><FiLink size={14} /></Button>}{can('edit') && <Button variant="outline-warning" onClick={() => handleEditCompte(compte)} style={{ borderColor: '#ffc107', color: '#ffc107' }}><FiEdit2 size={14} /></Button>}{can('edit') && <Button variant={compte.actif ? 'outline-secondary' : 'outline-success'} onClick={() => handleToggleActif(compte.id)} style={compte.actif ? { borderColor: '#6c757d', color: '#6c757d' } : { borderColor: '#198754', color: '#198754' }}>{compte.actif ? <FiXCircle size={14} /> : <FiCheckCircle size={14} />}</Button>}{can('delete') && !compte.obligatoire && <Button variant="outline-danger" onClick={() => handleDeleteCompte(compte.id)} style={{ borderColor: '#dc3545', color: '#dc3545' }}><FiTrash2 size={14} /></Button>}</>}
                          </div></td>
                        </tr>
                      ))}
                    </tbody>
          ...
          <div className="text-center mt-4">
            <small className="d-flex align-items-center justify-content-center gap-2" style={{ color: 'rgba(255,255,255,0.7)' }}>
              <FiShield size={12} /> Plan comptable conforme aux normes BCC — Mise à jour en temps réel
            </small>
          </div>
        </Container>
      </div>
